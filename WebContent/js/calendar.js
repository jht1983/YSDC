...
My97DP.prototype = {
	...
	update_month: function($) {
		...
		if ($ === undefined) $ = this.getNewDateStr();
		if($.length>7)$=$.substring(0,7);
		if ($dp.el[$dp.elProp] != $) $dp.el[$dp.elProp] = $;
		
		this.setRealValue_month()
	},
	...
	_fy: function(_, B, A) {
		...
		$.a(">¡û</td><td class='menu' onmouseover=\"this.className='menuOn'\" onmouseout=\"this.className='menu'\" onmousedown=\"hide($d.yD);$d.yI.blur();\">¡Á</td><td ");
	...
	mark: function(B, D) {
		...
		if($dp.errDealMode==0)$dp.errDealMode=-1;
...
function month_Click(F, B, _, D, C, A) {
	var $ = new DPDate($dt.y, $dt.M, $dt.d, $dt.H, $dt.m, $dt.s);
	...
	if (!callFunc("onpicking")) {
		...
		if ($c.autoPickDate || E || arguments.length == 0) $c.pickDate_month()
	} else $dt = $
}
...
DPDate.prototype = {
	...
	refresh: function() {
		...
		$d.okI.value=$lang.aMonStr[iMonth-1];
		this.d = $.getDate();
		this.H = $.getHours();
		this.m = $.getMinutes();
		this.s = $.getSeconds();
