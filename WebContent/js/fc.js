...
var f_sys_oneClick=0;



function doGraphClick(_strChartId){
	var dDbClickTime=new Date().getTime()-f_sys_oneClick;
	f_sys_oneClick=new Date().getTime();
	if(dDbClickTime>300)
		return;
	var objParentPageDoc=parent.document;
	var objCurGraph=objParentPageDoc.getElementById("sys_maxgraph");
	if(objCurGraph==null){
		var bgObj=objParentPageDoc.createElement("div");
		bgObj.style.width=objParentPageDoc.body.clientWidth;
		bgObj.style.height=objParentPageDoc.body.clientHeight;
		bgObj.setAttribute('id','sys_maxgraph');
		bgObj.style.backgroundColor='red';
		bgObj.style.position="absolute";
		bgObj.className="popgraphstyle";
		bgObj.style.top=objParentPageDoc.body.scrollTop;
		bgObj.style.left=objParentPageDoc.body.scrollLeft;
		bgObj.style.zIndex = "100002";
		bgObj.innerHTML="<iframe width='"+objParentPageDoc.body.clientWidth+"px' height='"+objParentPageDoc.body.clientHeight+"px' src='View?SPAGECODE="+spagecode+"&sys_full=true'></ifame>";
		objParentPageDoc.body.appendChild(bgObj);
	}else{
		objParentPageDoc.body.removeChild(objCurGraph);
	}
}
...
function doGraphClick_bak(_strChartId){
	var dDbClickTime=new Date().getTime()-f_sys_oneClick;
	f_sys_oneClick=new Date().getTime();
	if(dDbClickTime>300)
		return;
	var objCurGraph=$("sys_maxgraph");
	if(objCurGraph==null){
		...
		_strChartId.setAttribute('height', document.body.clientHeight+''); 
		_strChartId.setAttribute('width', document.body.clientWidth+''); 
		//alert(document.body.clientWidth);
		_strChartId.render("sys_maxgraph");
		_strChartId.setAttribute('height', document.body.clientHeight+''); 
		_strChartId.setAttribute('width', document.body.clientWidth+''); 
	}else{
		document.body.removeChild(objCurGraph);
	}
}