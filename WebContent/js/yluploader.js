(function (window, undefined) {
    ...
    var yltUploader = {
		...
		getFileIco:function getFileIco(a) {
    ...
    if (browser_ie) yltUploader["ie" + (browser_ie < 6 ? 6 : browser_ie)] = true;
    ...
    window.yltUploader = yltUploader;
...
(function (window, undefined) {
    ...
    var def = yltUploader.def,
        fire = yltUploader.fire,
        extend = yltUploader.extend,

        getFirst = yltUploader.getFirst,
        getLast = yltUploader.getLast,

        parseJSON = JSON.parse,

        createEle = yltUploader.createEle,
        parseHTML = yltUploader.parseHTML,

        setOpacity = yltUploader.setOpacity,
        getOffset = yltUploader.getOffset,

        md5File = yltUploader.md5File,
    ...
    */
    ...
    Uploader.prototype = {
        ...
        resetInput: function () {
            ...
            boxInput.innerHTML = '<input type="file" accept="'+self.strAllows+'" name="' + self.upName + '" style="' + (self.clickTrigger ? 'visibility: hidden;' : 'font-size:100px;') + '"' + (self.multiple ? ' multiple="multiple"' : '') + '>';
...
//alert(this.dataType);
    ...
    yltUploader.Uploader = Uploader;
})(window);
yltUploader.strOthParams="";
yltUploader.doChangeFielName=function(_obj,_strPath,_iCom){
	var strParams="path="+_strPath+"&nm="+_obj.attributes["oldname"].value+"&nnm="+_obj.value;
	if(_iCom!="-1"){
		var arrInputs=document.getElementsByTagName("select");
		var iInputCount=arrInputs.length;
		var strAllFiles="";
		var strSplit="";
		for(var i=0;i<iInputCount;i++){
			var objAttrCurPT=arrInputs[i].attributes["strcupt"];
			if(!!objAttrCurPT&&objAttrCurPT.value==_strPath){
				if(arrInputs[i].value!=""){
					strAllFiles+=strSplit+arrInputs[i].value;
					strSplit=",";
				}
			}
		}
		strParams+="&allfname="+strAllFiles+yltUploader.strOthParams;
	}
	var strResult=getTx(strParams,"Menu?O_SYS_TYPE=cgname&com="+_iCom);
	var iIndex=_obj.attributes["oldname"].value.lastIndexOf(".");
	var strExt="";
	if(iIndex!=-1)
		strExt=_obj.attributes["oldname"].value.substring(iIndex);
	_obj.attributes["oldname"].value=_obj.value+strExt;	
}
yltUploader.objChangeFile={};
yltUploader.setChangeFile=function(_strName,_strType,_strCom){
	yltUploader.objChangeFile[add.elements[_strName].value]=_strType+":"+_strCom;
};
yltUploader.changeFileName=function(_strPath,_fileName,_iCom){
	return "<input strcupt=\""+_strPath+"\" oldname=\""+_fileName+"\" onchange=\"yltUploader.doChangeFielName(this,'"+_strPath+"','"+_iCom+"')\" class='uploadname' type='text' value='"+_fileName.split(".")[0]+"'>";
};
yltUploader.changeDicFileName=function(_strPath,_fileName,_strDic,_iCom){
	var strSelValue=_fileName.split(".")[0];
	return "<select strcupt=\""+_strPath+"\" oldname=\""+_fileName+"\" onchange=\"yltUploader.doChangeFielName(this,'"+_strPath+"','"+_iCom+"')\" class='uploadname' type='text' value='"+strSelValue+"'>"+
	"<option>"+getTx("STROPTYPE="+_strDic+"&selv="+strSelValue,"DicManager")+"</option>"+
	"</select>";
}
yltUploader.initFileName=function(){
	var arrInputs=document.getElementsByTagName("div");
	var iInputCount=arrInputs.length;
	for(var i in yltUploader.objChangeFile){		
		for(var k=0;k<iInputCount;k++){
			var objPath=arrInputs[k].attributes["pt"];
			if(!!objPath){
				var strPath=objPath.value;
				if(i==strPath){
					var arrCgMsg=yltUploader.objChangeFile[i].split(":");
					var strFileName=arrInputs[k].innerHTML;
					if(arrCgMsg[0]=="1")
						strFileName=yltUploader.changeFileName(strPath,strFileName,arrCgMsg[1]);
					else
						strFileName=yltUploader.changeDicFileName(strPath,strFileName,arrCgMsg[0],arrCgMsg[1]);
					arrInputs[k].innerHTML=strFileName;
				}
			}
		}
...
function delMuiltsUpFile(_obj,_strFilePath,_strFileName){
	getTx("p="+_strFilePath+"&f="+_strFileName,"Menu?O_SYS_TYPE=delmupfile");
	var objParent=_obj.parentNode;
	objParent.parentNode.removeChild(objParent);
}
yltUploader.viewProgress=function(_strFilePath,_task){
	var strFileName=_task.name;
	var strIsChangeFN=yltUploader.objChangeFile[_strFilePath];
	if(!!strIsChangeFN){
		var arrCgMsg=strIsChangeFN.split(":");
		if(arrCgMsg[0]=="1")
			strFileName=yltUploader.changeFileName(_strFilePath,strFileName,arrCgMsg[1]);
		else
			strFileName=yltUploader.changeDicFileName(_strFilePath,strFileName,arrCgMsg[0],arrCgMsg[1]);
	}
	var a=this.getFileIco(_task.ext);
	var strGrees="<div class='uploadicoview'>"+
				 "<span id='divptext" + _task.id + "'></span><img id='img" + _task.id + "' src='images/file/" + a + "' width='22px' height='22px'>"+
				 "<span onclick=\"delMuiltsUpFile(this,'"+_strFilePath+"','"+_task.name+"');\" style='font-size:16px;cursor:pointer;'>&nbsp;X</span><br><div>"+strFileName+ "</div>"+
...
yltUploader.viewSingProgress=function(_strFilePath,_task){
	var a=this.getFileIco(_task.ext);
	var strIco="images/file/"+a;
	var strGrees="<div style='margin-left:10px;position:relative;float:left; width:120px;border:0px solid blue;text-align:center;'>"+
				 "<span id='divptext" + _task.id + "'></span><img id='img" + _task.id + "' src='"+strIco+"' width='22px' height='22px'>"+
				 "<br><div style='margin-top:3px;width:120px;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;height:16px;line-height:16px;'>"+ _task.name + "</div>"+
				 "<div id='pro" + _task.id + "' style='position:absolute;right:0px;top:0px;background:rgba(69,198,247,0.2);width:100%;height:100%;' id='pro" + _task.id + "'></div></div>";
	$("view"+_strFilePath).innerHTML=strGrees;
...
function initSingFile(_strFilePath,_strCode){
	new yltUploader.Uploader({
            ...
            UI: {
                draw: function (task) {
					yltUploader.viewSingProgress(_strFilePath,task);
...
function initMutFile(_strFilePath){
	new yltUploader.Uploader({
            url: uploadUrl,
            target: document.getElementById("btn"+_strFilePath),
			data: {
            ...
            UI: {
                draw: function (task) {
					yltUploader.viewProgress(_strFilePath,task);
...
yltUploader.FreeOpSucces=function(){
	window.location.reload();
}
function initSingUpload_FreeOp(_isysType,_fileSavePath,_fileName,_maxSize,_ext,_strDeScript,_strUploadBttnId,_strParam1,_strParam2){
		new yltUploader.Uploader({
            url: uploadUrl,
            target: document.getElementById(_strUploadBttnId),
			data: {
					isystype:_isysType,
					filepath:_fileSavePath,
					fnself:_fileName,
					strparam1:_strParam1,
					strparam2:_strParam2
				  },
			allows: _ext,
            multiple: true,
			on: {
