...
function createTree(p_div,p_backColor,p_contextMenu) {
	var tree = {
		...
		createNode: function(p_text,p_expanded, p_icon, p_parentNode,p_tag,p_contextmenu,_stype) {
			v_tree = this;
			node = {
				...
				createChildNode: function(p_text,p_expanded,p_icon,p_tag,p_contextmenu,_stype) { return v_tree.createNode(p_text,p_expanded,p_icon,this,p_tag,p_contextmenu,_stype); }
			}
			...
			if (this.rendered) {
				if (p_parentNode==undefined) {
					this.drawNode(this.ulElement,node,_stype);
					...
					if (p_parentNode.childNodes.length==0) {
						...
						v_img.src = 'res/images/collapse.png';
						v_img.id = 'toggle_off';
						}
						else {
							...
							v_img = p_parentNode.elementLi.getElementsByTagName("img")[0];
							v_img.style.visibility = "visible";
							v_img.src = 'res/images/expand.png';
							v_img.id = 'toggle_on';
						}
					}
					this.drawNode(v_ul,node,_stype);
		...
		drawTree: function() {
			...
			for (var i=0; i<this.childNodes.length; i++) {
				this.drawNode(ulElement,this.childNodes[i],true);
			}
      ...
      this.adjustLines(document.getElementById(this.name),true);
		...
		drawNode: function(p_ulElement,p_node,_type) {
			...
			var v_exp_col = null;
            if(_type==true){
                v_exp_col = createImgElement('toggle_on', 'exp_col', 'res/images/expand.png');
            }else{
    			if (p_node.childNodes.length == 0) {
    				v_exp_col = createImgElement('toggle_off','exp_col','res/images/collapse.png');
		...
		expandNode: function(p_node) {
			if (p_node.childNodes.length>0 && p_node.expanded==false) {
				...
				img.id="toggle_off";
				img.src = 'res/images/collapse.png';
